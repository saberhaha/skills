...
CHECK_INTERVAL_SECONDS="${SKILL_RUNTIME_CHECK_INTERVAL_SECONDS:-1800}"
...
sync_skill() {
  local now_epoch local_updated_before local_updated_after should_check_remote
  ...
  if helm skill pull "${SKILL_NAME}" >/dev/null 2>&1; then
    sync_local_pre_execute_from_registry
    local_updated_after="$(read_frontmatter_value "${SKILL_FILE}" updated 2>/dev/null || true)"
    CURRENT_LOCAL_UPDATED="${local_updated_after:-${local_updated_before}}"
    if [ -n "${local_updated_before}" ] && [ -n "${local_updated_after}" ] && [ "${local_updated_before}" != "${local_updated_after}" ]; then
      echo "[pre-execute] updated: skill=${SKILL_NAME}, from=${local_updated_before}, to=${local_updated_after}"
    else
      echo "[pre-execute] checked: skill=${SKILL_NAME}, updated=${CURRENT_LOCAL_UPDATED:-unknown}"
    fi
  else
