...
def main():
    ...
    # --- 数据导出 ---
    p = add_common(sub.add_parser("export")); p.add_argument("--sql", required=True); p.add_argument("--filetype", default="excel", choices=["excel", "csv"]); p.add_argument("--output", help="输出文件路径"); p.add_argument("--engine", default="SPARK", choices=["SPARK", "PRESTO", "HIVE"], help="查询引擎（默认 SPARK）")
    ...
    try:
        ...
        elif cmd == "query": result = dp.query(args.sql, engine=args.engine)
        ...
        elif cmd == "export": result = dp.export_dump(args.sql, args.filetype, args.output, engine=args.engine)
