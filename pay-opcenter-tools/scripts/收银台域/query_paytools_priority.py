"""
PayTools优先级配置 — 查询PayTools优先级配置规则
来源：schemas/收银台域/v3_PayTools优先级配置.json
"""
import sys
import os
...
# 入参 Schema（来源：v3_PayTools优先级配置.json）
PARAM_SCHEMA = {
    "type": {
        "type": "number",
        "required": True,
        "default": 2,
        "description": "数据类型固定值：CashierBizDataType.PayListPriotiry=2（页面固定，无需用户传入；注：源码枚举名有拼写错误 Priotiry）"
    },
    "id": {
        "type": "number",
        "required": False,
...
# 数据展示列（来源：v3_PayTools优先级配置.json → display）
# 规则卡片视图，非标准表格，每条规则包含：
#   scenarios: 场景列表
#   value: 支付工具规则集配置
#   desc: 备注
...
DISPLAY_COLUMNS = [
    ...
    {"title": "支付工具规则集配置", "dataIndex": "value"},
...
def execute(
    ...
    """
    TYPE = 2  # CashierBizDataType.PayListPriotiry = 2（注：源码拼写错误）
