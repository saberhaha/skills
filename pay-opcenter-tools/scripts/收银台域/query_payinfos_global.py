"""
PayInfos全局配置 — 查询PayInfos全局配置规则
来源：schemas/收银台域/v3_PayInfos全局配置.json
"""
import sys
import os
...
# 入参 Schema（来源：v3_PayInfos全局配置.json）
PARAM_SCHEMA = {
    "type": {
        "type": "number",
        "required": True,
        "default": 5,
        "description": "数据类型固定值：CashierBizDataType.PayInfoGlobal=5（页面固定，无需用户传入）"
    },
    "id": {
        "type": "number",
        "required": False,
...
# 数据展示列（来源：v3_PayInfos全局配置.json → display）
# 规则卡片视图，非标准表格，每条规则包含：
#   scenarios: 场景列表
#   value: PayInfo配置值
#   desc: 备注
...
def execute(
    ...
    """
    TYPE = 5  # CashierBizDataType.PayInfoGlobal = 5
