"""
收银台类型管理 — 查询收银台类型配置规则
来源：schemas/收银台域/v3_收银台类型管理.json
"""
import sys
import os
...
# 入参 Schema（来源：v3_收银台类型管理.json）
PARAM_SCHEMA = {
    "type": {
        "type": "number",
        "required": True,
        "default": 4,
        "description": "数据类型固定值：CashierBizDataType.CashierType=4（页面固定，无需用户传入）"
    },
    "id": {
        "type": "number",
        "required": False,
...
# 数据展示列（来源：v3_收银台类型管理.json → display）
# 规则卡片视图，非标准表格，每条规则包含：
#   scenarios: 场景列表
#   value: 收银台类型字符串（如 "A"）
#   desc: 备注
...
def execute(
    ...
    """查询收银台类型配置规则
    不传 rule_id 时返回当前已发布的规则列表（status 接口）；
    传入 rule_id 时返回该规则的详情（detail 接口）。

    Args:
        rule_id: 规则ID。可选。不传则查询已发布规则列表，传入则查询单条规则详情。
    """
    TYPE = 4  # CashierBizDataType.CashierType = 4
