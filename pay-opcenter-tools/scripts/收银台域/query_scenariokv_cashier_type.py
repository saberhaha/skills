"""
场景KV-收银台类型名单 — 查询场景KV收银台类型名单列表
来源：schemas/收银台域/v3_场景KV收银台类型.json
"""
import sys
import os
...
# 入参 Schema（来源：v3_场景KV收银台类型.json）
PARAM_SCHEMA = {
...
# 数据展示列（来源：v3_场景KV收银台类型.json → display.columns）
DISPLAY_COLUMNS = [
...
def execute(
    ...
    """
    url = f"{BASE_URL}/v3/api/cashier-biz-manage/scenariokvs/type/cashier-type/names"
