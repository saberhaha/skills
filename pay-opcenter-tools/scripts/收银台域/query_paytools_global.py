"""
PayTools全局配置 — 查询PayTools全局配置规则
来源：schemas/收银台域/v3_PayTools全局配置.json
"""
import sys
import os
...
# 入参 Schema（来源：v3_PayTools全局配置.json）
PARAM_SCHEMA = {
    "type": {
        "type": "number",
        "required": True,
        "default": 1,
        "description": "数据类型固定值：CashierBizDataType.PayListGlobal=1（页面固定，无需用户传入）"
    },
    "id": {
        "type": "number",
        "required": False,
...
# 数据展示列（来源：v3_PayTools全局配置.json → display）
# 规则卡片视图，非标准表格，每条规则包含：
#   scenarios: 场景列表
#   value: 支付工具规则集配置
#   desc: 备注
...
def execute(
    ...
    """查询PayTools全局配置规则
    不传 rule_id 时返回当前已发布的全局配置规则列表（status 接口）；
    传入 rule_id 时返回该规则的详情（detail 接口）。

    Args:
        rule_id: 规则ID。可选。不传则查询已发布规则列表，传入则查询单条规则详情。
    """
    TYPE = 1  # CashierBizDataType.PayListGlobal = 1
