"""
PayInfos版本 — 查询PayInfos版本列表
来源：schemas/收银台域/v3_PayInfos版本.json
"""
import sys
import os
...
# 入参 Schema（来源：v3_PayInfos版本.json）
PARAM_SCHEMA = {
    "type": {
        "type": "enum",
        "required": False,
        "default": 5,
        "enum_values": {5: "PayInfos全局匹配(PayInfoGlobal)", 3: "PayInfos优先匹配(PayInfoPriority)"},
        "description": "版本类型，默认为5（全局匹配）"
    },
...
# 数据展示列（来源：v3_PayInfos版本.json → display.columns）
DISPLAY_COLUMNS = [
...
def execute(
    ...
    Args:
        type: 版本类型。可选。枚举：5=PayInfos全局匹配, 3=PayInfos优先匹配。默认=5。
