"""
用户快捷支付绑定银行卡查询(v3) — 用户快捷支付绑定银行卡查询(v3)
来源：schemas/商户域/v3_用户绑卡列表.json
"""
import sys
import os
...
DISPLAY_COLUMNS = [
    ...
    {"title": "发行卡", "dataIndex": "bankName"},
    {"title": "后四位", "dataIndex": "cardNoLastFour"},
    {"title": "手机号", "dataIndex": "mobile"},
    {"title": "绑卡时间", "dataIndex": "updated", "format": "YYYY-MM-DD HH:mm:ss"},
]
...
def execute(
    ...
    """用户快捷支付绑定银行卡查询（v3）
    根据手机号和银行卡后四位查询用户绑定的银行卡列表。
    v3版本参数包装在singleParam对象中。无分页，返回全量结果。
